// Hash: Y2055l5gr+kAlN3uEOXcdme8H1VysPjV5f8DUkHiSQh3SVa1cD7jWVhCvCi2/dGpH5Ib+wbgbr3M/8nGcMG2FPm0eKt+sqLTKHVW5pltvkRVwmpNFk3LFivnfOVPD4sRcgThnZo783siXjMEpXlZ/AbyftUoBYr9OGPUS+dFGLs=
(function () {
    'use strict';
    ...
    var Log = /** @class */ (function () {
        ...
        Log.Log = function (level, msg) {
            ...
            var label = LogLevel[level].toUpperCase();
            var ua = window.navigator.userAgent;
            var isIE = ua.indexOf("MSIE ") > 0 || ua.indexOf("Trident/") > 0;
            if (window.console) {
                if (isIE) {
                    // IE does not support colored logs
                    console.log("[PubTag] " + label + ": " + msg);
                }
                else {
                    console.log("%cPubTag", CSS_LOG, label + ": " + msg);
                }
            }
            else if (level <= LogLevel.Error) {
                alert("[PubTag] " + label + ": " + msg);
            }
        };
        ...
        if (!cmpInFrame(currentWindow)) {
            ...
            currentWindow.__cmp = function (cmpCommand, cmpParameter, cmpCallback) {
                if (!cmpFrame_1) {
                    ...
                    cmpCallback({ msg: "CMP not found" }, false);
                    return;
                }
                ...
                cmpCallbacks[callId] = cmpCallback;
    ...
    function getCMPFrame(currentWindow) {
        ...
        var cmpFrame;
        // We try to find the CMP in at most 10 parent frames, that way we ensure we will not loop indefinitely
        // (safeguard if frame === currentWindow.top is never true)
        for (var i = 0; i < 10; ++i) {
            try {
    ...
    /**
     * Implementation of the lock algorithm from Leslie Lamport, which was designed to be fast in the cases where there is
     * no contention, which is usually the case for web (rarely do multiple pages access the localStorage at exactly the
     * same time).
     *
     * @see https://www.microsoft.com/en-us/research/uploads/prod/2016/12/A-Fast-Mutual-Exclusion-Algorithm.pdf
     */
    var Lock = /** @class */ (function () {
        ...
        Lock.prototype.get = function (key) {
            ...
            if (splitted.length < 2 || parseInt(splitted[1], 10) < now()) {
                return undefined;
            }
            return splitted[0];
        };
        Lock.prototype.set = function (key, expiration) {
            var exp = now() + expiration;
            localStorage.setItem(key, this.id + "|" + exp);
        };
        Lock.prototype.acquire = function (callback, expiration, timeout) {
            var _this = this;
            if (timeout === void 0) { timeout = 100; }
            if (timeout <= 0 || this.tryAcquire(expiration)) {
                callback();
                this.release();
            }
            else {
                setTimeout(function () {
                    _this.acquire(callback, expiration, timeout - 10);
                }, 10);
            }
        };
        Lock.prototype.tryAcquire = function (expiration) {
            localStorage.setItem(this.x, this.id);
            var owner = this.get(this.y);
            if (!owner || owner === this.id) {
                this.set(this.y, expiration);
                // No contention or won lock
                if (localStorage.getItem(this.x) === this.id && this.get(this.y) === this.id) {
                    return true;
                }
            }
            return false;
        ...
        Lock.prototype.release = function () {
            localStorage.removeItem(this.x); // Not necessary, but done to keep the user's localStorage clean
    ...
    var SlotKey = /** @class */ (function () {
        function SlotKey(impressionId) {
            this.impressionId = impressionId;
        }
        SlotKey.prototype.toString = function () {
    ...
    var SlotKeyWithSize = /** @class */ (function (_super) {
        __extends(SlotKeyWithSize, _super);
        function SlotKeyWithSize(impressionId, size, networkId) {
            var _this = _super.call(this, impressionId) || this;
            _this.size = size;
            _this.networkId = networkId;
            return _this;
        }
        SlotKeyWithSize.prototype.toString = function () {
            return _super.prototype.toString.call(this) + ("_Size" + this.size + "_NetworkId" + this.networkId);
        };
        return SlotKeyWithSize;
    }(SlotKey));
    ...
    var SlotKeyWithZoneId = /** @class */ (function (_super) {
        __extends$1(SlotKeyWithZoneId, _super);
        function SlotKeyWithZoneId(impressionId, zoneId) {
            var _this = _super.call(this, impressionId) || this;
            _this.zoneId = zoneId;
            return _this;
        }
        SlotKeyWithZoneId.prototype.toString = function () {
            return _super.prototype.toString.call(this) + ("_ZoneId" + this.zoneId);
        };
        return SlotKeyWithZoneId;
    }(SlotKey));

    var SlotKeyFactory = /** @class */ (function () {
        function SlotKeyFactory(useZoneIdIntegration, networkId) {
            this.useZoneIdIntegration = useZoneIdIntegration;
            this.networkId = networkId;
        }
        SlotKeyFactory.prototype.createKeysFromSlotRequest = function (slot) {
            if (this.useZoneIdIntegration) {
                return [new SlotKeyWithZoneId(slot.impId, slot.zoneId)];
            }
            else {
                var keys = [];
                for (var _i = 0, _a = slot.sizes; _i < _a.length; _i++) {
                    var size = _a[_i];
                    keys.push(new SlotKeyWithSize(slot.impId, size, this.networkId));
                }
                return keys;
            }
        };
        SlotKeyFactory.prototype.createKeyFromSlotResponse = function (slot) {
            if (this.useZoneIdIntegration) {
                return new SlotKeyWithZoneId(slot.impid, slot.zoneid);
            }
            else {
                return new SlotKeyWithSize(slot.impid, new Size(slot.width, slot.height), this.networkId);
            }
        };
        SlotKeyFactory.prototype.createKeyFromBid = function (bid) {
            if (this.useZoneIdIntegration) {
                return new SlotKeyWithZoneId(bid.impressionId, bid.zoneId);
            }
            else {
                return new SlotKeyWithSize(bid.impressionId, new Size(bid.width, bid.height), this.networkId);
            }
        };
        return SlotKeyFactory;
    }());
    ...
    var DirectBiddingBidManager = /** @class */ (function () {
        function DirectBiddingBidManager(useZoneIdIntegration, networkId) {
            this.localStorageHelper = new LocalStorageHelper();
            this.localStorageEnabled = this.localStorageHelper.checkLocalStorage();
            this.slotKeyFactory = new SlotKeyFactory(useZoneIdIntegration, networkId);
        }
        ...
        DirectBiddingBidManager.prototype.get = function (key) {
            var fullKey = DirectBiddingBidManager.BID_KEY_PREFIX + key.toString();
            var raw = this.localStorageHelper.getItem(fullKey);
            var value = raw && tryParseJson(raw);
            if (value && typeof value === "object" && "bids" in value) {
                value.bids = value.bids.filter(function (bid) { return bid.expiration > now(); });
                return value;
            }
            return { bids: [] };
        };
        DirectBiddingBidManager.prototype.set = function (key, val) {
            var fullKey = DirectBiddingBidManager.BID_KEY_PREFIX + key.toString();
            if (val.bids.length > 0 || (val.no_bid && val.no_bid > now())) {
                this.localStorageHelper.setItem(fullKey, JSON.stringify(val));
            }
            else {
                this.localStorageHelper.removeItem(fullKey);
            }
        };
        DirectBiddingBidManager.prototype.filterNoBidSlots = function (slots) {
            ...
            for (var _i = 0, slots_1 = slots; _i < slots_1.length; _i++) {
                var slot = slots_1[_i];
                var sizes = [];
                for (var _a = 0, _b = this.slotKeyFactory.createKeysFromSlotRequest(slot); _a < _b.length; _a++) {
                    var key = _b[_a];
                    var bid = this.getBid(key, 0);
                    if (bid !== DirectBiddingBidManager.NO_BID) {
                        if (key instanceof SlotKeyWithSize) {
                            sizes.push(key.size);
                        }
                        else {
                            ret.push(slot);
        ...
        DirectBiddingBidManager.prototype.getRequestCachedBids = function (slots, duration) {
            ...
            for (var _i = 0, slots_2 = slots; _i < slots_2.length; _i++) {
                ...
                for (var _a = 0, _b = this.slotKeyFactory.createKeysFromSlotRequest(slot); _a < _b.length; _a++) {
                    var key = _b[_a];
                    var bid = this.getBid(key, duration);
        ...
        DirectBiddingBidManager.prototype.getBid = function (key, duration) {
            if (duration === void 0) { duration = 5000; }
            if (this.localStorageEnabled) {
                var cache = this.get(key);
                if (cache.no_bid && cache.no_bid > now()) {
                    return DirectBiddingBidManager.NO_BID;
                }
                if (duration > 0) {
                    for (var _i = 0, _a = cache.bids; _i < _a.length; _i++) {
                        var bid = _a[_i];
                        var lock = new Lock(DirectBiddingBidManager.BID_KEY_PREFIX + bid.bid.slotid);
                        if (lock.tryAcquire(duration)) {
                            return bid.bid;
                        }
                    }
                }
        ...
        DirectBiddingBidManager.prototype.storeRequestNoBid = function (slot, bidCacheDurationSeconds) {
            for (var _i = 0, _a = this.slotKeyFactory.createKeysFromSlotRequest(slot); _i < _a.length; _i++) {
                var key = _a[_i];
        ...
        DirectBiddingBidManager.prototype.storeResponseBid = function (slot, bidCacheDurationSeconds) {
            var key = this.slotKeyFactory.createKeyFromSlotResponse(slot);
            this.modifyCache(key, function (cache) {
                cache.bids.push({
                    bid: slot,
                    expiration: now() + bidCacheDurationSeconds * 1000
                });
            });
        };
        DirectBiddingBidManager.prototype.storeNoBid = function (key, storageDurationInSeconds) {
            this.modifyCache(key, function (cache) {
                cache.no_bid = Math.max(cache.no_bid || 0, now() + storageDurationInSeconds * 1000);
            });
        };
        DirectBiddingBidManager.prototype.removeBid = function (bid) {
            var key = this.slotKeyFactory.createKeyFromBid(bid);
            this.modifyCache(key, function (cache) {
                for (var i = 0; i < cache.bids.length; i++) {
                    if (cache.bids[i].bid.slotid === bid.slotId) {
                        cache.bids.splice(i, 1);
                        return;
                    }
                }
            });
            var lock = new Lock(key.toString());
            lock.release();
        };
        DirectBiddingBidManager.prototype.modifyCache = function (key, callback) {
            var _this = this;
            if (this.localStorageEnabled) {
                var lock_1 = new Lock(key.toString());
                lock_1.acquire(function () {
        ...
        DirectBiddingBidManager.BID_KEY_PREFIX = "criteo_pt_cdb_bidcache_";
        return DirectBiddingBidManager;
    }());

    var DirectBiddingCache = /** @class */ (function () {
        function DirectBiddingCache(context, slots, networkId) {
        ...
        DirectBiddingCache.prototype.filterNoBidSlots = function (slots) {
            var filteredSlots = this.bidManager.filterNoBidSlots(slots);
            // If silent mode is ignored we dont rely on cache and return all slots
            if (this.context.shouldIgnoreSilentMode) {
                if (filteredSlots.length !== slots.length && this.context.setSilentModeIgnored) {
        ...
        DirectBiddingCache.prototype.silentModeEnabled = function () {
            ...
            if (this.silentModeManager.silentModeEnabled()) {
                if (this.context.shouldIgnoreSilentMode && this.context.setSilentModeIgnored) {
                    this.context.setSilentModeIgnored();
                }
                else {
        ...
        DirectBiddingCache.prototype.removeBid = function (bid) {
            this.bidManager.removeBid(bid);
        };
        DirectBiddingCache.prototype.handleResponse = function (requestSlots, response, extraData, timeouted) {
            ...
            if (response.slots) {
                for (var _b = 0, _c = response.slots; _b < _c.length; _b++) {
                    var slot = _c[_b];
                    var ttl = 0;
                    if (slot.slotid in ttls) {
                        ttl = ttls[slot.slotid];
                        delete ttls[slot.slotid];
                    }
                    if (timeouted && ttl > 0) {
            ...
            for (var slotId in ttls) {
                if (ttls.hasOwnProperty(slotId)) {
    ...
    var DirectBiddingMetric = /** @class */ (function () {
        function DirectBiddingMetric(publisherTagVersion, slots, elapsed, isTimeout, pageLoadElapsed, adapterStartElapsed, cdbCallStartElapsed, cdbCallEndElapsed, adapterEndElapsed, setTargetingElapsed, adapterTimeout, adapterIsTimeout) {
            this.publisherTagVersion = publisherTagVersion;
            this.slots = slots;
    ...
    var DirectBiddingMetricSlot = /** @class */ (function () {
        function DirectBiddingMetricSlot(impressionId, // Impression ID generated by CDB
        zoneId, adUnitId, cachedBidUsed) {
            this.impressionId = impressionId;
            this.zoneId = zoneId;
            this.adUnitId = adUnitId;
            this.cachedBidUsed = cachedBidUsed;
        }
        return DirectBiddingMetricSlot;
    }());
    ...
    var DirectBiddingMetricBuilder = /** @class */ (function () {
        ...
        DirectBiddingMetricBuilder.prototype.withPageLoadElapsed = function (pageLoadElapsed) {
            this.pageLoadElapsed = Math.round(pageLoadElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withAdapterStartElapsed = function (adapterStartElapsed) {
            this.adapterStartElapsed = Math.round(adapterStartElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withCdbCallStartElapsed = function (cdbCallStartElapsed) {
            this.cdbCallStartElapsed = Math.round(cdbCallStartElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withCdbCallEndElapsed = function (cdbCallEndElapsed) {
            this.cdbCallEndElapsed = Math.round(cdbCallEndElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withSetTargetingElapsed = function (setTargetingElapsed) {
            this.setTargetingElapsed = Math.round(setTargetingElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withAdapterEndElapsed = function (adapterEndElapsed) {
            this.adapterEndElapsed = Math.round(adapterEndElapsed);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withAdapterTimeout = function (adapterTimeout) {
            this.adapterTimeout = adapterTimeout && Math.round(adapterTimeout);
            return this;
        };
        DirectBiddingMetricBuilder.prototype.withCachedBidsReturned = function (cachedBidsReturned) {
            this.cachedBidsReturned = cachedBidsReturned;
            return this;
        };
        DirectBiddingMetricBuilder.prototype.addSlot = function (impId, zoneId, adUnitId) {
            var cachedBidUsed = this.cachedBidsReturned
                .filter(function (bid) { return bid.impid === adUnitId && bid.zoneid === zoneId; })
                .length > 0;
            this.slots.push(new DirectBiddingMetricSlot(impId, zoneId, adUnitId, cachedBidUsed));
            return this;
        };
        DirectBiddingMetricBuilder.prototype.build = function () {
            var adapterIsTimeout;
            if (this.adapterTimeout !== undefined) {
                adapterIsTimeout = this.adapterEndElapsed > this.adapterTimeout;
            }
            return new DirectBiddingMetric(PublisherTagVersion, this.slots, this.elapsed, this.isTimeout, this.pageLoadElapsed, this.adapterStartElapsed, this.cdbCallStartElapsed, this.cdbCallEndElapsed, this.adapterEndElapsed, this.setTargetingElapsed, this.adapterTimeout, adapterIsTimeout);
        };
    ...
    var DirectBiddingMetricsManager = /** @class */ (function () {
        ...
        DirectBiddingMetricsManager.prototype.storeMetric = function (metric) {
            if (this.localStorageEnabled) {
                var metrics = this.getMetrics(false);
                metrics.push(metric);
    ...
    var DirectBiddingRequestBuilder = /** @class */ (function () {
        function DirectBiddingRequestBuilder(slots, context, metricsManager, urlBuilder, profileId, integrationMode, networkId, adapterVersion, gdprConsent, wrapperVersion) {
            this.slots = slots;
            this.context = context;
            this.metricsManager = metricsManager;
            this.urlBuilder = urlBuilder;
            this.profileId = profileId;
            this.integrationMode = integrationMode || IntegrationMode.Unspecified;
            this.networkId = networkId;
    ...
    var DirectBiddingTimer = /** @class */ (function () {
        function DirectBiddingTimer(metricBuilder, auctionStart, timeout) {
        ...
        DirectBiddingTimer.prototype.finish = function (slots) {
            this.builder.withAdapterEndElapsed(this.timer.elapsed());
            if (!slots || slots.length === 0) {
                this.builder.addSlot("");
            }
            else {
                for (var _i = 0, slots_1 = slots; _i < slots_1.length; _i++) {
                    var slot = slots_1[_i];
                    this.builder.addSlot(slot.imp_id, slot.zone_id, slot.ad_unit_id);
                }
            }
            return this.build();
        };
        DirectBiddingTimer.prototype.build = function () {
    ...
    var DirectBiddingUrlBuilder = /** @class */ (function () {
        ...
        DirectBiddingUrlBuilder.prototype.buildUrl = function (profileId, context, integrationMode, adapterVersion, wrapperVersion) {
            ...
            if (wrapperVersion !== undefined) {
                biddingUrl += "&wv=" + encodeURIComponent(wrapperVersion);
            }
            // silent mode ignored
            biddingUrl += context.silentModeIgnored ? "&smi=1" : "";
            // random number
    ...
    function resolvePrebidTimeout(requestTimeout) {
        var prebidTimeout = typeof window.PREBID_TIMEOUT === "number" ? window.PREBID_TIMEOUT : undefined;
    ...
    var Prebid = /** @class */ (function () {
        function Prebid(profileId, adapterVersion, bidRequests, bidderRequest, prebidVersion) {
            this.timer = new DirectBiddingTimer(new DirectBiddingMetricBuilder(), bidderRequest.auctionStart, resolvePrebidTimeout(bidderRequest.timeout));
            ...
            this.cache = new DirectBiddingCache(window.criteo_pubtag.context, this.slots, networkId);
            this.requestBuilder = new DirectBiddingRequestBuilder(this.cache.filterNoBidSlots(this.slots), window.criteo_pubtag.context, this.metricsManager, new DirectBiddingUrlBuilder(false), profileId, integrationMode, networkId, adapterVersion, gdprConsent, prebidVersion);
        ...
        Prebid.prototype.interpretResponse = function (response, request) {
            ...
            if (response.slots && Array.isArray(response.slots)) {
                for (var _b = 0, _c = response.slots; _b < _c.length; _b++) {
                    ...
                    if (!bidRequest) {
                        Log.Error("Could not get bid request for slot \"" + slot + "\"");
                        continue;
                    }
                    ...
                    var creative = slot.native
                        ...
                        : slot.creative;
                    var ttl = slot.ttl
                        || (slotsMap[slot.slotid] && slotsMap[slot.slotid].ttl)
                        || 60;
                    bids.push({
                        requestId: bidId,
                        adId: generateUuid(),
            ...
            this.metricsManager.storeMetric(this.timer.finish(extraData.slots));
            return bids;
        };
        Prebid.prototype.handleBidWon = function (bid) {
            this.updateMetric(bid, function (slot) {
                slot.adapterBidWon = true;
            });
        };
        ...
        Prebid.prototype.handleSetTargeting = function (bid) {
            var _this = this;
            this.timer.setTargeting();
            this.updateMetric(bid, function () { return _this.timer.build(); });
        };
        Prebid.prototype.updateMetric = function (bid, callback) {
            var metrics = this.metricsManager.getMetrics(false);
            for (var i = 0; i < metrics.length; ++i) {
                for (var _i = 0, _a = metrics[i].slots; _i < _a.length; _i++) {
                    var slot = _a[_i];
                    if (slot.adUnitId === bid.adUnitCode) {
                        var ret = callback(slot);
                        if (ret) {
                            metrics[i] = ret;
                        }
                    }
    ...
    var AsyncRequest = /** @class */ (function () {
        ...
        AsyncRequest.prototype.getXDomainRequest = function (method, onSuccess, onError, onTimeout, timeout) {
            if (typeof XDomainRequest !== "undefined") {
                ...
                if (request_1.ontimeout && onTimeout) {
    ...
    var DirectBiddingEvent = /** @class */ (function (_super) {
        ...
        function DirectBiddingEvent(profileId, urlBuilder, slots, callbackSuccess, callbackError, callbackTimeout, timeout, networkId, integrationMode, adapterVersion) {
            ...
            _this.slots = slots;
            _this.metricBuilder = new DirectBiddingMetricBuilder();
        ...
        DirectBiddingEvent.prototype.getMetricBuilder = function () {
            return this.metricBuilder;
        };
        DirectBiddingEvent.prototype.eval = function (publishertag) {
            this.evalWithTimeout(publishertag, undefined);
        };
        DirectBiddingEvent.prototype.evalWithTimeout = function (publishertag, timeout) {
            var _this = this;
            ...
            var adapterTimeout = timeout || resolvePrebidTimeout(bidRequest && bidRequest.timeout);
            var timer = new DirectBiddingTimer(this.metricBuilder, auctionStart, adapterTimeout);
            ...
            request.send(function (response) {
                ...
                _this.metricsManager.storeMetric(timer.finish(extraData.slots));
                ...
                // Once adapter callback is called we consider adapter has ended
                _this.metricsManager.storeMetric(timer.finish());
            }, function () {
    ...
    var DirectBiddingEventWithCaching = /** @class */ (function (_super) {
        ...
        function DirectBiddingEventWithCaching(profileId, urlBuilder, slots, callbackSuccess, callbackError, callbackTimeout, timeout, networkId, integrationMode, adapterVersion) {
            ...
            var httpTimeout = Math.max((timeout || 3000) * 10, 3000);
            _this.cache = new DirectBiddingCache(window.criteo_pubtag.context, slots, networkId);
        ...
        DirectBiddingEventWithCaching.prototype.evalWithCmp = function (publishertag, gdprConsent) {
            ...
            if (this.cache.silentModeEnabled()) {
                ...
                // Configure bid caching callback to get the opportunity to return bids in case of timeout
                setTimeout(function () { return _this.onTimeout(); }, this.timeout || 3000);
            }
        ...
        DirectBiddingEventWithCaching.prototype.onSuccess = function (bidResponse, extraData) {
            ...
            if (extraData !== undefined) {
                var response = tryParseJson(bidResponse);
        ...
        DirectBiddingEventWithCaching.prototype.onTimeout = function () {
            ...
            if (cachedBids.length === 0) {
                ...
                this.directBiddingEvent.getMetricBuilder().withCachedBidsReturned(cachedBids);
                this.callbackSuccess(JSON.stringify({ slots: cachedBids }), undefined);
            }
        };
        DirectBiddingEventWithCaching.prototype.getMetricBuilder = function () {
            return this.directBiddingEvent.getMetricBuilder();
        };
        DirectBiddingEventWithCaching.prototype.getBidCache = function () {
            return this.cache;
        };
    ...
    function updateFastBid(type) {
        var expiration = 24 * 60 * 60 * 1000; // Expires after one day
        var ls = new LocalStorageHelper();
        var lsKey = FASTBID_KEY[type];
        // If local storage is not available or if fast bid is existing and not expired, do nothing
        if (!ls.checkLocalStorage() || ls.getItem(lsKey, expiration) !== null) {
            return;
        }
        var updateRequest = new AsyncRequest("//static.criteo.net/js/ld/publishertag." + type + ".js", undefined, undefined, false);
        updateRequest.send(function (data) {
            ls.setItem(lsKey, data, expiration);
        }, function (state, status) {
            Log.Error("Could not update FastBid" + (status ? " (" + status + ")" : ""));
        });
    }
    ...
    (function (StorageOrigin) {
        StorageOrigin[StorageOrigin["None"] = 0] = "None";
        StorageOrigin[StorageOrigin["Cookie"] = 1] = "Cookie";
        StorageOrigin[StorageOrigin["LocalStorage"] = 2] = "LocalStorage";
    ...
    var CookieSynchronizer = /** @class */ (function () {
        ...
        CookieSynchronizer.isAndroidBrowser = function () {
            return navigator.userAgent.toLowerCase().indexOf("android") > -1;
        };
        CookieSynchronizer.isFirefoxBrowser = function () {
            return navigator.userAgent.toLowerCase().indexOf("firefox") > -1;
        };
        CookieSynchronizer.prototype.synchronizeCriteoUid = function (forceSyncframe) {
            var _this = this;
            // Only drop syncframe on safari browsers, android, and firefox browsers
            if (!forceSyncframe
                && !CookieSynchronizer.isSafariBrowser()
                && !CookieSynchronizer.isAndroidBrowser()
                && !CookieSynchronizer.isFirefoxBrowser()) {
                return;
            }
            if (this.topWin.addEventListener) {
                if (this.topDoc.readyState === "complete") {
                    ...
                    // in some cases, the DOMContentLoaded event is not triggered, and we need to wait for the load
                    // event of the window
        ...
        CookieSynchronizer.prototype.appendGumIframeIfDoesNotExist = function () {
            ...
            if (!this.topDoc.getElementById(CookieSynchronizer.SYNCFRAME_ID)) {
                this.topWin.addEventListener("message", evtListener, true);
        ...
        CookieSynchronizer.prototype.getClientSideOptOut = function () {
            ...
            return {
                value: optout.value === "1",
                origin: optout.origin
            };
        };
        ...
        CookieSynchronizer.prototype.getLocalWebId = function () {
            ...
            var lwidRead = this.getFromAllStorages(CookieSynchronizer.LOCAL_WEB_ID_COOKIE_NAME);
            // if does not exist, it means we are either on a tracker/non fpc domain or it's the first time
            // the user arrives on the website
            if (this.canWriteCookies) {
                var lwidToWrite = lwidRead.value;
                if (!lwidToWrite) {
                    lwidToWrite = generateUuid();
                }
                // if we can write a cookie, write a local web id on all storages. On safari >= 12.2, the javascript cookie
                // TTL will be capped at 7 days
                this.writeOnAllStorages(CookieSynchronizer.LOCAL_WEB_ID_COOKIE_NAME, String(lwidToWrite), CookieSynchronizer.GUID_RETENTION_TIME_HOUR);
                lwidRead = this.getFromAllStorages(CookieSynchronizer.LOCAL_WEB_ID_COOKIE_NAME);
            }
            return (lwidRead.value && lwidRead) || { value: "NA", origin: StorageOrigin.None };
        };
        // check if we can write a cookie on Publisher domain
        CookieSynchronizer.prototype.checkCookiesAreWriteable = function () {
            var checkWritableCookieName = "cto_writeable";
            CookieHelper.SetCookie(checkWritableCookieName, "1", 1, this.topDoc, true);
            var canWrite = CookieHelper.GetCookie(checkWritableCookieName, this.topDoc) === "1";
            CookieHelper.DeleteCookie(checkWritableCookieName, this.topDoc, true);
            return canWrite;
        };
        ...
        CookieSynchronizer.prototype.getFromAllStorages = function (key) {
            ...
            if (this.localStorageEnabled) {
                fromLocalStorage = this.localStorageHelper.getItem(key) || undefined;
            }
            // tslint:disable-next-line:no-bitwise
            var origin = (fromPubCookie && StorageOrigin.Cookie) | (fromLocalStorage && StorageOrigin.LocalStorage);
            var value = (fromPubCookie || fromLocalStorage);
    ...
    var DomManipulationTools = /** @class */ (function () {
        ...
        DomManipulationTools.getHighestAccessibleUrl = function (highestAccessibleWindowStruct) {
            var highestAccessibleWindow = highestAccessibleWindowStruct.topFrame;
            var crossDomainError = highestAccessibleWindowStruct.err;
            // If we managed to get the top window without any cross-domain error, we can directly return its URL
            if (!crossDomainError) {
                return highestAccessibleWindow.location.href;
            }
            // Try to access to top window directly (will throw if the bottom and top domain are different)
            try {
                var topLocation = highestAccessibleWindow.top.location.href;
                if (topLocation) {
                    return topLocation;
                }
            }
            catch (e) { }
            // Try to use the non-standard 'ancestorOrigins' API implemented in Chrome, Safari, and Firefox
            try {
                var aOrigins = highestAccessibleWindow.location.ancestorOrigins;
                if (aOrigins) {
                    // The origins are in bottom to top order, so the top domain is the last entry in the array
                    return aOrigins[aOrigins.length - 1];
                }
            }
            catch (e) { }
            // If nothing else worked, we return the referrer
            return highestAccessibleWindow.document.referrer;
        };
    ...
    var Context = /** @class */ (function () {
        ...
        function Context(document, window) {
            ...
            this.highestAccessibleUrl = DomManipulationTools.getHighestAccessibleUrl(highestAccessibleWindowStruct);
            /* Sync criteo uid */
        ...
        Context.prototype.synchronizeCriteoUid = function (highestAccessibleWindowStruct, highestAccessibleUrl) {
            ...
            this.ctoIdOnPublisherDomain = soc.getClientSideUid().value; // read uid from all storages
            this.isOptOut = soc.getClientSideOptOut().value; // read optout status from all storages
            this.idfs = soc.getClientSideIdfs().value; // read idfs from all storages
            this.secureId = soc.getClientSideSecureId().value; // read secureId from all storages
        ...
        Context.prototype.computeShouldIgnoreSilentMode = function () {
            var silentModeIgnoredPercentage = 5;
            // Cache is ignored 5% of the time
            return Math.floor(Math.random() * 100) < silentModeIgnoredPercentage;
        };
        Context.prototype.setSilentModeIgnored = function () {
            this.silentModeIgnored = true;
        };
        return Context;
    }());
            ...
            while (i < this.events.length) {
                ...
                if (isConditionalEvent(event_2) && !event_2.canEval()) {
                    ...
                    try {
                        ...
                        Log.Error("An exception occurred processing an event: " + e.toString());
                    }
                }
            }
        };
        ...
        for (var key in obj) {
            if (obj.hasOwnProperty(key)) {
                ...
                if (typeof elt === "function") {
                    obj[key] = safeFunction(elt);
                }
                else if (typeof elt === "object") {
                    obj[key] = safeObject(elt);
                }
            }
        }
        return obj;
